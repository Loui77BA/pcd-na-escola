...
/* .video-duration removida (elementos não usados) */
...
/* Acessibilidade: duplicação de .sr-only removida (definida no topo) */
...
<div class="video-page-container">
  <!-- Coluna Principal do Vídeo -->
  <main class="video-main-column" aria-labelledby="current-video-title">
    <section aria-labelledby="current-video-title" aria-describedby="video-accessibility-note">
      ...
      <div class="video-info" id="video-info">
        <h2 id="current-video-title" aria-live="polite">Aula 01 - Tema Escuro e Temas de Contraste</h2>
  ...
  <aside class="video-sidebar" aria-labelledby="video-list-heading">
    <h3 id="video-list-heading">Vídeos do Curso</h3>
    
    <nav aria-labelledby="video-list-heading">
      <ul class="video-list" id="video-list">
      <!-- Vídeo 1 -->
      <li class="video-list-item active">
        <a href="#" aria-controls="main-video"
...
function changeVideo(event, element) {
  ...
  // Indicar atualização em andamento
  const info = document.getElementById('video-info');
  if (info) info.setAttribute('aria-busy', 'true');
  ...
  // Mudança de vídeo será anunciada via aria-live no heading
  
  // Focar no player (opcional, melhora navegação por teclado)
  player.focus();

  // Finaliza atualização
  if (info) info.setAttribute('aria-busy', 'false');
}
...
function toggleDescription() {
  ...
  if (isExpanded) {
    // Colapsar: esconder do leitor de telas
    ...
    // Expandir: expor ao leitor de telas
